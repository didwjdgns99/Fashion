...
export default function RecentOrder({ orderData }: SuccessCardProps) {
  console.log("오더데이터입니다앙아아아", orderData);
  if (!orderData.order) {
  ...
  return (
    <div className="h-100 overflow-hidden overflow-y-auto flex flex-col gap-2">
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          <div key={orderData.order.orderId} className="flex gap-4">
          ...
          <span className="font-bold">
            {orderData.order.items.length > 1
              ? `${orderData.order.items[0].name} 외 ${
                  orderData.order.items.length - 1
                }개`
        ...
        <div className="flex justify-between border-t-1  py-2">
          <span className="text-[15px] text-black font-bold">
            총 {orderData.order.items.length} 개의 상품
          </span>
          <span className=" font-semibold">
            {orderData.order.totalPrice.toLocaleString()} 원
          </span>
        </div>
      </div>
    </div>
